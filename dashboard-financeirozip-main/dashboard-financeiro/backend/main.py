...
@app.get("/api/diagnostico/posicao-saldos")
def diag_posicao_saldos(data: Optional[str] = None, empresa: Optional[int] = None):
    """DEBUG: inspeciona tabela posicao_saldos."""
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        # Descobre a ultima data disponivel
        cursor.execute("SELECT MAX(data_movimento) as ultima FROM posicao_saldos")
        ultima = cursor.fetchone()
        ultima_data = ultima['ultima'].strftime('%Y-%m-%d') if ultima and ultima['ultima'] else None

        # Datas distintas recentes
        cursor.execute("SELECT DISTINCT data_movimento FROM posicao_saldos ORDER BY data_movimento DESC LIMIT 15")
        datas = [r['data_movimento'].strftime('%Y-%m-%d') for r in cursor.fetchall()]

        # Empresas distintas
        cursor.execute("SELECT DISTINCT id_interno_empresa FROM posicao_saldos ORDER BY id_interno_empresa")
        empresas_ids = [r['id_interno_empresa'] for r in cursor.fetchall()]

        # Amostra de nomes (tipos de conta)
        cursor.execute("SELECT DISTINCT nome FROM posicao_saldos ORDER BY nome LIMIT 50")
        nomes_amostra = [r['nome'] for r in cursor.fetchall()]

        # Amostra para data/empresa especifica
        data_q = data or ultima_data
        amostra = []
        if data_q:
            params = [data_q]
            emp_sql = ""
            ...
            cursor.execute(f"""
                ...
                FROM posicao_saldos
                WHERE data_movimento = %s {emp_sql}
                ORDER BY id_interno_empresa, id_conta_corrente
                LIMIT 50
            """, params)
        ...
        return {
            ...
            'datas_recentes': datas,
            'empresas_ids': empresas_ids,
            'nomes_amostra': nomes_amostra,
            'amostra': amostra,
        }
    except Exception as e:
        import traceback
