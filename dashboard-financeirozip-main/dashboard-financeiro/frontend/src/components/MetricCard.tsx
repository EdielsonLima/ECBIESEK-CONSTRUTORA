...
const colorClasses = {
  green: 'bg-white dark:bg-slate-800 text-gray-800 dark:text-slate-200 border-green-100 shadow-sm',
  blue: 'bg-white dark:bg-slate-800 text-gray-800 dark:text-slate-200 border-blue-100 shadow-sm',
  red: 'bg-white dark:bg-slate-800 text-gray-800 dark:text-slate-200 border-red-100 shadow-sm',
};
  ...
  return (
    <div className={`rounded-2xl border p-6 ${colorClasses[color]} transition-all duration-300 hover:-translate-y-1 hover:shadow-lg`}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">{title}</p>
          <p className="mt-2 text-3xl font-extrabold text-gray-900 dark:text-slate-100">{value}</p>
