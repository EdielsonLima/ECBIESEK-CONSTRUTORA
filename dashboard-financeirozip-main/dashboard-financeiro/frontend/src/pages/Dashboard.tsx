...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* ============ GRAFICOS ============ */}
      <div className="animate-slide-up-delay-3 grid gap-6 lg:grid-cols-2">
        {/* Evolucao Mensal */}
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-gray-800">Evolucao Mensal</h3>
            <span className="text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded-full">{graficoMensal.length} meses</span>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={chartData}>
              ...
              <XAxis dataKey="mesFormatado" tick={{ fontSize: 10, fill: '#9ca3af' }} />
              ...
              <Area type="monotone" dataKey="pago" stroke="#10b981" strokeWidth={2} fill="url(#gradPago)" name="Pago" dot={false} />
              <Area type="monotone" dataKey="a_pagar" stroke="#3b82f6" strokeWidth={2} fill="url(#gradAPagar)" name="A Pagar" dot={false} />
              <Line type="monotone" dataKey="em_atraso" stroke="#ef4444" strokeWidth={2} name="Em Atraso" dot={false} strokeDasharray="5 5" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded-full">Top {topCategories.length}</span>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={topCategories} layout="vertical" margin={{ left: 10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} />
              <XAxis type="number" tickFormatter={formatCurrencyShort} tick={{ fontSize: 10, fill: '#9ca3af' }} />
              <YAxis dataKey="categoria" type="category" width={100} tick={{ fontSize: 10, fill: '#6b7280' }} />
              <Tooltip
                formatter={(value: number) => formatCurrency(value)}
                contentStyle={{ borderRadius: '12px', border: '1px solid #e5e7eb', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
              />
          ...
          <div className="rounded-2xl border border-red-100 bg-white shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-red-50 to-white px-6 py-4 border-b border-red-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-base font-bold text-gray-800">Contas em Atraso</h3>
              </div>
              <button onClick={() => nav('contas-atrasadas')} className="text-xs text-red-600 font-semibold hover:text-red-800 transition-colors flex items-center gap-1">
                Ver todas
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gray-50/50">
                    <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Fornecedor</th>
                    <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Vencimento</th>
                    <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Valor</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {contasEmAtraso.slice(0, 7).map((conta, i) => {
                    const hoje = new Date();
                    hoje.setHours(0, 0, 0, 0);
                    const [a, m, d] = (conta.data_vencimento || '').split('T')[0].split('-').map(Number);
                    ...
                    return (
                      <tr key={i} className="hover:bg-red-50/50 transition-colors">
                        <td className="px-4 py-2.5 text-sm text-gray-700 truncate max-w-[200px]">{conta.credor || conta.fornecedor || '-'}</td>
                        <td className="px-4 py-2.5 text-sm">
                          <span className="text-gray-600">{formatDate(conta.data_vencimento)}</span>
                          <span className="ml-1 text-xs text-red-500 font-semibold">({dias}d)</span>
                        </td>
                        <td className="px-4 py-2.5 text-sm font-bold text-red-600 text-right">{formatCurrency(conta.valor_total || conta.valor || 0)}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="rounded-2xl border border-blue-100 bg-white shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-blue-50 to-white px-6 py-4 border-b border-blue-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-base font-bold text-gray-800">Proximos Vencimentos</h3>
              </div>
              <button onClick={() => nav('contas-a-pagar')} className="text-xs text-blue-600 font-semibold hover:text-blue-800 transition-colors flex items-center gap-1">
                Ver todas
                <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </button>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {proximosVencimentos.slice(0, 7).map((conta, i) => {
                    const hoje = new Date();
                    hoje.setHours(0, 0, 0, 0);
                    const [a, m, d] = (conta.data_vencimento || '').split('T')[0].split('-').map(Number);
                    const venc = new Date(a, m - 1, d);
                    ...
                    return (
                      <tr key={i} className="hover:bg-blue-50/50 transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-sm">
                          <span className="text-gray-600">{formatDate(conta.data_vencimento)}</span>
                          <span className={`ml-1 text-xs font-semibold ${dias <= 3 ? 'text-red-500' : dias <= 7 ? 'text-amber-500' : 'text-blue-500'}`}>({dias}d)</span>
                        </td>
                        <td className="px-4 py-2.5 text-sm font-bold text-gray-700 text-right">{formatCurrency(conta.valor_total || conta.valor || 0)}</td>
                      </tr>
                    );
                  })}
                </tbody>
