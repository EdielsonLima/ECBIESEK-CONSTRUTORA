...
const CORES_ACAO: Record<string, string> = {
  ...
  REMOVER_USUARIO: 'bg-red-100 text-red-700',
};
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{filtradas.length} registros</p>
          </div>
          <select
            value={filtroAcao}
                ...
                <tbody>
                  {pagAtual.map(a => (
                    <tr key={a.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-2.5 text-gray-600 whitespace-nowrap text-xs">{formatarData(a.created_at)}</td>
                      <td className="px-6 py-2.5 font-medium text-gray-800">{a.email}</td>
                      <td className="px-6 py-2.5">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${CORES_ACAO[a.acao] ?? 'bg-gray-100 text-gray-600'}`}>
                          {a.acao}
                        </span>
                      </td>
                      <td className="px-6 py-2.5 text-gray-500 text-xs">{a.detalhes ?? '—'}</td>
